		...
		switch (command) {
			...
			case "Exit":
				StringBuilder sb = new StringBuilder()
						.append(String.format("Cryo Reactors: %d%s", this.repository.getReactors().values().stream()
								.filter(x -> x.getClass().getSimpleName().toUpperCase().contains(ReactorType.CRYO.toString())).count(), System.lineSeparator()))
						.append(String.format("Heat Reactors: %d%s", this.repository.getReactors().values().stream()
								.filter(x -> x.getClass().getSimpleName().toUpperCase().contains(ReactorType.HEAT.toString())).count(), System.lineSeparator()))
						.append(String.format("Energy Modules: %d%s", this.repository.getIdentifiables().values().stream()
								.map(x -> x.getClass().getSuperclass().getInterfaces())
								.filter(x -> x.length > 0).map(x -> x[0].getSimpleName())
								.filter(x -> x.equals("EnergyModule")).count(), System.lineSeparator()))
						.append(String.format("Absorbing Modules: %d%s", this.repository.getIdentifiables().values().stream()
								.map(x -> x.getClass().getSuperclass().getInterfaces())
								.filter(x -> x.length > 0).map(x -> x[0].getSimpleName())
								.filter(x -> x.equals("AbsorbingModule")).count(), System.lineSeparator()))
						.append(String.format("Total Energy Output: %d%s", this.repository.getReactors().values().stream()
								.mapToLong(x -> x.getTotalEnergyOutput() > x.getTotalHeatAbsorbing() ? 0 : x.getTotalEnergyOutput()).sum(), System.lineSeparator()))
						.append(String.format("Total Heat Absorbing: %d", this.repository.getReactors().values().stream()
